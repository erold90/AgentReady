...
<div style="margin:24px 0;padding:20px;background:#fffbeb;border:1px solid #fde68a;border-radius:8px;text-align:center">
  ...
  const missingFiles = [];
  if (!protocols?.a2a?.found) missingFiles.push({ name: 'agent.json', path: '/.well-known/agent.json', code: JSON.stringify({ name: (ps.title || domain) + ' Agent', description: 'AI agent for ' + domain, url: 'https://' + domain, version: '1.0.0', capabilities: { streaming: false, pushNotifications: false }, skills: [{ name: 'general_info', description: 'Get information about ' + domain, tags: ['info','help'] }], authentication: null }, null, 2) });
  if (!protocols?.mcp?.found) missingFiles.push({ name: 'mcp.json', path: '/.well-known/mcp.json', code: JSON.stringify({ mcpServers: { [domain.replace(/\./g,'-')]: { url: 'https://' + domain + '/mcp', transport: 'streamable-http', description: 'MCP server for ' + domain } } }, null, 2) });
  if (!protocols?.agents?.found) missingFiles.push({ name: 'agents.json', path: '/.well-known/agents.json', code: JSON.stringify({ agents: [{ name: (ps.title || domain) + ' Assistant', description: 'AI assistant for ' + domain, protocol: 'a2a', url: 'https://' + domain + '/.well-known/agent.json', capabilities: ['chat','search'] }] }, null, 2) });
  if (!protocols?.llms?.found) missingFiles.push({ name: 'llms.txt', path: '/llms.txt', code: `# ${ps.title || domain}\n\n> ${domain}\n\n## Pages\n- [Home](https://${domain}/)\n- [About](https://${domain}/about)\n- [Contact](https://${domain}/contact)` });
  missingFiles.push({ name: 'robots.txt', path: '/robots.txt', code: `User-agent: *\nAllow: /\n\nUser-agent: GPTBot\nAllow: /\n\nUser-agent: ChatGPT-User\nAllow: /\n\nUser-agent: ClaudeBot\nAllow: /\n\nUser-agent: Google-Extended\nAllow: /\n\nUser-agent: PerplexityBot\nAllow: /\n\nSitemap: https://${domain}/sitemap.xml` });

  if (missingFiles.length > 0) {
    html += '<h2>Generated Code</h2><p style="color:#475569;font-size:13px;margin-bottom:16px">Copy these files to make your site AI Agent-Ready:</p>';
    missingFiles.forEach(f => {
      html += `<div class="action"><div class="action-title">${esc(f.path)}</div><pre>${esc(f.code)}</pre></div>`;
    });
  }
